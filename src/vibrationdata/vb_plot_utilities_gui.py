...
################################################################################
    ...
    def PerformAnalysis(self):
        ...
        if (n == m):
            from vibrationdata.vb_time_history_plot_gui import vb_time_history_plot
            vb_time_history_plot(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_scatter_plot_gui import vb_scatter_plot
            vb_scatter_plot(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_psd_plot_gui import vb_psd_plot
            vb_psd_plot(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_srs_plot_gui import vb_srs_plot
            vb_srs_plot(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_spl_plot_gui import vb_spl_plot
            vb_spl_plot(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_ft_mp_plot_gui import vb_ft_mp_plot
            vb_ft_mp_plot(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_ft_ri_plot_gui import vb_ft_ri_plot
            vb_ft_ri_plot(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_nyquist_nichols_plots_gui import vb_nyquist_nichols_plots
            vb_nyquist_nichols_plots(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_two_curves_plots_gui import vb_two_curves_plots
            vb_two_curves_plots(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_multiple_curves_gui import vb_multiple_curves
