...
from vibrationdata.vb_utilities import WriteData2, WriteData3, sample_rate_check, \
...
#        self.master.geometry("600x600")
        ...
        self.hwtext1 = tk.Label(
            top, text='Shock Response Spectrum for Base Excitation')
        self.hwtext1.grid(row=0, column=0, columnspan=6, pady=10, sticky=tk.W)

        self.hwtext2 = tk.Label(
            top, text='The input file must have two columns:  time(sec) & accel(G)')
...
###############################################################################
        ...
        self.button_read.grid(
            row=2, column=0, columnspan=1, pady=20, sticky=tk.W)
...
###############################################################################
        ...
        self.button_calculate = tk.Button(
            top, text="Calculate", command=self.srs_calculation)
        self.button_calculate.config(height=2, width=18, state='disabled')
        self.button_calculate.grid(
            row=crow, column=0, columnspan=1, padx=3, pady=20)

        self.button_tripartite = tk.Button(
            top, text="Plot Tripartite", command=self.tripartite)
        self.button_tripartite.config(height=2, width=18, state='disabled')
        self.button_tripartite.grid(
            row=crow, column=1, columnspan=1, padx=3, pady=20)
...
################################################################################
    ...
    def read_data(self):
        ...
        try:
            self.a, self.b, self.num =\
                read_two_columns_from_dialog(
                    'Select Acceleration File', self.master)
        except:
            tkMessageBox.showwarning(
                "Warning", "Input file must have two columns with no header lines")
        ...
        self.sr, self.dt = sample_rate_check(
            self.a, self.b, self.num, self.sr, self.dt)
    ...
    def srs_calculation(self):
        ...
        self.a_pos, self.a_neg, self.a_abs = \
            vb_SRS.accel_SRS(self.b, self.num_fn,
                             self.omega, self.damp, self.dt)

        nLb1 = int(self.Lb1.curselection()[0])

        self.rd_pos, self.rd_neg, self.rd_abs, self.pv_pos, self.pv_neg, self.pv_abs =\
            self.rd_SRS(self.b, self.num_fn, self.omega,
                        self.damp, self.dt, nLb1)
        ...
        if (n == 0):
            ...
            plt.plot(self.fn, self.pv_abs, linewidth=1.0,
                     color='b')        # disregard error
...
################################################################################
    ...
    def export_accel(self):
        ...
        if (n == 0):
            WriteData3(self.num_fn, self.fn, self.a_pos,
                       self.a_neg, output_file)
        else:
    ...
    def export_pv(self):
        ...
        n = int(self.Lb2.curselection()[0])
        if (n == 0):
            WriteData3(self.num_fn, self.fn, self.pv_pos,
                       self.pv_neg, output_file)
        else:
    ...
    def export_rd(self):
        ...
        n = int(self.Lb2.curselection()[0])
        if (n == 0):
            WriteData3(self.num_fn, self.fn, self.rd_pos,
                       self.rd_neg, output_file)
        else:
...
#     theta=-34
        ...
        if (amin <= 0.01 and amax >= 0.01):
            plt.text(fmin, 0.012*scale/(2*np.pi*fmin),
                     '0.01 G', fontsize=12, rotation=theta)
            k += 1

        if (amin <= 0.1 and amax >= 0.1):
            plt.text(fmin, 0.12*scale/(2*np.pi*fmin),
                     '0.1 G', fontsize=12, rotation=theta)
            k += 1

        if (amin <= 1 and amax >= 1):
            plt.text(fmin, 1.2*scale/(2*np.pi*fmin),
                     '1 G', fontsize=12, rotation=theta)
            k += 1

        if (amin <= 10 and amax >= 10):
            plt.text(fmin, 12*scale/(2*np.pi*fmin),
                     '10 G', fontsize=12, rotation=theta)
            k += 1

        if (amin <= 100 and amax >= 100):
            plt.text(fmin, 120*scale/(2*np.pi*fmin),
                     '100 G', fontsize=12, rotation=theta)
            k += 1

        if (amin <= 1000 and amax >= 1000):
            plt.text(fmin, 1200*scale/(2*np.pi*fmin),
                     '1K G', fontsize=12, rotation=theta)
            k += 1

        if (amin <= 10000 and amax >= 10000):
            plt.text(fmin, 12000*scale/(2*np.pi*fmin),
                     '10K G', fontsize=12, rotation=theta)
            k += 1

        if (amin <= 100000 and amax >= 100000):
            plt.text(fmin, 120000*scale/(2*np.pi*fmin),
                     '100K G', fontsize=12, rotation=theta)
            k += 1

#
        if (amax <= 0.01 and 0.01 < aamax):
            plt.text(0.012*scale/(2*np.pi*ymax), 0.9*ymax,
                     '0.01 G', fontsize=12, rotation=theta)
            k += 1

        if (amax <= 0.1 and 0.1 < aamax):
            plt.text(0.12*scale/(2*np.pi*ymax), 0.9*ymax,
                     '0.1 G', fontsize=12, rotation=theta)
            k += 1

        if (amax <= 1 and 1 < aamax):
            plt.text(1.2*scale/(2*np.pi*ymax), 0.9*ymax,
                     '1 G', fontsize=12, rotation=theta)
            k += 1

        if (amax <= 10 and 10 < aamax):
            plt.text(12*scale/(2*np.pi*ymax), 0.9*ymax,
                     '10 G', fontsize=12, rotation=theta)
            k += 1

        if (amax <= 100 and 100 < aamax):
            plt.text(120*scale/(2*np.pi*ymax), 0.9*ymax,
                     '100 G', fontsize=12, rotation=theta)
            k += 1

        if (amax <= 1000 and 1000 < aamax):
            plt.text(1200*scale/(2*np.pi*ymax), 0.9*ymax,
                     '1K G', fontsize=12, rotation=theta)
            k += 1

        if (amax <= 10000 and 10000 < aamax):
            plt.text(12000*scale/(2*np.pi*ymax), 0.9*ymax,
                     '10K G', fontsize=12, rotation=theta)
            k += 1

        if (amax <= 100000 and 100000 < aamax):
            plt.text(120000*scale/(2*np.pi*ymax), 0.9*ymax,
                     '100K G', fontsize=12, rotation=theta)
            k += 1
...
#            vs='cm/sec'
        ...
        if (dmin <= 10 and dmax >= 10):
            plt.text(fmax32, 4*tpfmax, '10 ' + str("%s" %
                     ds), fontsize=12, rotation=theta)

        if (dmin <= 1 and dmax >= 1):
            plt.text(fmax32, 0.4*tpfmax, '1 ' + str("%s" %
                     ds), fontsize=12, rotation=theta)

        if (dmin <= 0.1 and dmax >= 0.1):
            plt.text(fmax32, 0.04*tpfmax, '0.1 ' + str("%s" %
                     ds), fontsize=12, rotation=theta)

        if (dmin <= 0.01 and dmax >= 0.01):
            plt.text(fmax32, 0.004*tpfmax, '0.01 ' + str("%s" %
                     ds), fontsize=12, rotation=theta)

        if (dmin <= 0.001 and dmax >= 0.001):
            plt.text(fmax32, 0.0004*tpfmax, '0.001 ' +
                     str("%s" % ds), fontsize=12, rotation=theta)

        if (dmin <= 0.0001 and dmax >= 0.0001):
            plt.text(fmax32, 0.00004*tpfmax, '0.0001 ' +
                     str("%s" % ds), fontsize=12, rotation=theta)
        ...
        if (ddmin <= 0.01 and 0.01 < ddmax):
            plt.text(b*ymax/(2*np.pi*0.01), sc*ymax, '0.01 ' +
                     str("%s" % ds), fontsize=12, rotation=theta)

        if (ddmin <= 0.1 and 0.1 < ddmax):
            plt.text(b*ymax/(2*np.pi*0.1), sc*ymax, '0.1 ' +
                     str("%s" % ds), fontsize=12, rotation=theta)

        if (ddmin <= 1 and 1 < ddmax):
            plt.text(b*ymax/(2*np.pi*1), sc*ymax, '1 ' +
                     str("%s" % ds), fontsize=12, rotation=theta)

        if (ddmin <= 10 and 10 < ddmax):
            plt.text(b*ymax/(2*np.pi*10), sc*ymax, '10 ' +
                     str("%s" % ds), fontsize=12, rotation=theta)

        if (ddmin <= 100 and 100 < ddmax):
            plt.text(b*ymax/(2*np.pi*100), sc*ymax, '100 ' +
                     str("%s" % ds), fontsize=12, rotation=theta)
