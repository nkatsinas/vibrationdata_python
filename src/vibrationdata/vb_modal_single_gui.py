...
from vibrationdata.vb_utilities import WriteData2, signal_stats, sample_rate_check
...
###############################################################################
        ...
        self.hwtext3 = tk.Label(
            top, text='This script synthesizes a time history for a force or pressure PSD.')
        ...
        self.hwtext3 = tk.Label(
            top, text='The input file must have two columns: Freq(Hz) & psd(units^2/Hz)')
        ...
        self.button_read.grid(row=crow, column=2,
        ...
        self.button_ex = tk.Button(
            top, text="Export Time History", command=self.export)
        self.button_ex.config(height=2, width=23, state='disabled')
        self.button_ex.grid(row=crow, column=2, columnspan=2, padx=10, pady=3)

        self.button_ex_psd = tk.Button(
            top, text="Export PSD", command=self.export_psd)
        self.button_ex_psd.config(height=2, width=14, state='disabled')
        self.button_ex_psd.grid(
            row=crow, column=4, columnspan=1, padx=10, pady=3)
...
###############################################################################
        ...
        plt.plot(self.freq_spec, self.amp_spec *
                 sqrt(2), color='k', label='+/-1.5 dB')
    ...
    def read_data(self):
        ...
        if not self.durr.get():  # do something
            tkMessageBox.showinfo(
                "Warning", "Enter duration", parent=self.button_read)
            return
