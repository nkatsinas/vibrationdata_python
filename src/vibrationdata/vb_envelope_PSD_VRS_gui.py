...
###############################################################################
        ...
        self.hwtext3 = tk.Label(
            top, text='This script generates a simplified, optimized PSD to envelope a PSD.')
        self.hwtext3.grid(row=crow, column=0, columnspan=5,
                          pady=6, sticky=tk.W)

        crow = crow+1

        self.hwtext4 = tk.Label(
            top, text='The script assumes an SDOF response.')
        self.hwtext4.grid(row=crow, column=0, columnspan=5,
                          pady=0, sticky=tk.W)

        crow = crow+1

        self.hwtext5 = tk.Label(
            top, text='The input file must have two columns: Freq(Hz) & Accel(G**2/Hz)')
        self.hwtext5.grid(row=crow, column=0, columnspan=2,
                          pady=6, sticky=tk.W)

        crow = crow+1

        self.hwtext6 = tk.Label(top, text='Select Envelope PSD Type')
        self.hwtext6.grid(row=crow, column=1, columnspan=1,
                          pady=6, sticky=tk.S)
        ...
        self.button_read.grid(row=crow, column=0,
                              columnspan=1, padx=0, pady=2, sticky=tk.N)
        ...
        self.hwtext_Q.grid(row=crow, column=0, columnspan=1,
                           padx=14, pady=10, sticky=tk.S)

        self.hwtext_trials = tk.Label(top, text='Enter Number of Trials')
        self.hwtext_trials.grid(
            row=crow, column=1, columnspan=1, padx=14, pady=10, sticky=tk.S)
        ...
        self.ntrials_entry = tk.Entry(
            top, width=12, textvariable=self.ntrialsr)
        self.ntrials_entry.grid(
            row=crow, column=1, padx=14, pady=1, sticky=tk.N)
        ...
        self.hwtext11.grid(row=crow, column=1, columnspan=1,
                           pady=10, sticky=tk.S)
...
######
        ...
        self.hwtext21.grid(row=crow, column=1, columnspan=1,
                           pady=10, sticky=tk.S)
...
###############################################################################
    ...
    def read_data(self):
        ...
        f, a, num = read_two_columns_from_dialog(
...
###############################################################################
    ...
    def export(self):
        ...
        output_file_path = asksaveasfilename(
            parent=root, title="Enter the output filename")
...
#        ffmin=self.f1
        ...
        if (min(self.psdin) <= 0.):
            tkMessageBox.showinfo(
                "Warning", " Input error:  each PSD amplitude must be > 0.", parent=self.button_calculate)
            return

        if (min(self.fin) < 0.):
            tkMessageBox.showinfo(
                "Warning", " Input error:  each frequency must be > 0.", parent=self.button_calculate)
            return
        ...
        for i in range(1, len(self.f_ref)):
            ...
            for j in range(0, (len(self.fin)-1)):
                ...
                if (self.f_ref[i] >= self.fin[j] and self.f_ref[i] <= self.fin[j+1]):
                    ...
                    self.interp_psdin.append(
                        self.psdin[j]*((self.f_ref[i]/self.fin[j])**self.inslope[j]))
...
#
            ...
            print('   Trial: drms=%10.4g  vrms=%10.4g  grms=%10.4g ' %
                  (self.drms, self.vrms, self.grms))
...
###############################################################################
        ...
        plt.plot(self.f, self.a, label='Input ' +
                 str("%6.3g" % self.grms_in)+' GRMS')
        plt.plot(self.xf, self.xapsd, label='Envelope ' +
                 str("%6.3g" % self.grmsp)+' GRMS')

        title_string = 'Power Spectral Density   ' + \
            str("%6.3g" % self.grms_in)+' GRMS Overall '
        plt.title(title_string)
        plt.ylabel(' Accel (G^2/Hz)')
        plt.xlabel(' Frequency (Hz) ')
        plt.grid(which='both')
...
###############################################################################
    ...
    def env_interp_best(cls, self):
        ...
        for i in range(0, (self.nbreak-1)):
            ...
            self.xslope[i] = log(self.xapsd[i+1]/self.xapsd[i]
                                 )/log(self.xf[i+1]/self.xf[i])
        ...
        for i in range(0, self.nref):
            ...
            for j in range(0, (self.nbreak-1)):
                ...
                if ((self.xffine[i] >= self.xf[j]) and (self.xffine[i] <= self.xf[j+1])):
                    ...
                    self.xapsdfine[i] = self.xapsd[j] * \
                        ((self.xffine[i] / self.xf[j])**self.xslope[j])
                    break
...
###############################################################################
    ...
    def env_grms_sam(cls, self):
        ...
        for i in range(0, (self.nbreak-1)):
            ...
            s[i] = log(self.apsd_sam[i+1]/self.apsd_sam[i]) / \
                log(self.f_sam[i+1]/self.f_sam[i])

            if (s[i] < -1.0001 or s[i] > -0.9999):
                ra = ra + (self.apsd_sam[i+1] * self.f_sam[i+1] -
                           self.apsd_sam[i]*self.f_sam[i])/(s[i]+1.)
            else:
                ra = ra + self.apsd_sam[i]*self.f_sam[i] * \
                    log(self.f_sam[i+1]/self.f_sam[i])
...
###############################################################################
    ...
    def env_vrms_sam(cls, self):
        ...
        for i in range(0, (self.nbreak-1)):
            ...
            s[i] = log(vpsd_sam[i+1]/vpsd_sam[i]) / \
                log(self.f_sam[i+1]/self.f_sam[i])

            if (s[i] < -1.0001 or s[i] > -0.9999):
                ra = ra + (vpsd_sam[i+1] * self.f_sam[i+1] -
                           vpsd_sam[i]*self.f_sam[i])/(s[i]+1.)
            else:
                ra = ra + vpsd_sam[i]*self.f_sam[i] * \
                    log(self.f_sam[i+1]/self.f_sam[i])
 ...
 ###############################################################################
    ...
    def env_drms_sam(cls, self):
        ...
        for i in range(0, (self.nbreak-1)):
            ...
            s[i] = log(dpsd_sam[i+1]/dpsd_sam[i]) / \
                log(self.f_sam[i+1]/self.f_sam[i])

            if (s[i] < -1.0001 or s[i] > -0.9999):

                ra = ra + (dpsd_sam[i+1] * self.f_sam[i+1] -
                           dpsd_sam[i]*self.f_sam[i])/(s[i]+1.)

            else:
                ra = ra + dpsd_sam[i]*self.f_sam[i] * \
                    log(self.f_sam[i+1]/self.f_sam[i])
    ...
    def env_compare_rms(cls, self):
        ...
        for i in range(0, self.nref):
            ...
            if (self.vrs_samfine[i] < 1.0e-30):
                print('\n Error:  vrs_samfine[%d])=%10.4g ' % (
                    i, self.vrs_samfine[i]))
                return
...
###############################################################################
    ...
    def env_interp_sam(cls, self):
        ...
        for i in range(0, (self.nbreak-1)):
            self.slope[i] = log(self.apsd_sam[i+1]/self.apsd_sam[i]
                                )/log(self.f_sam[i+1]/self.f_sam[i])
        ...
        for i in range(0, self.nref):
            ...
            for j in range(0, (self.nbreak-1)):
                ...
                if ((self.f_samfine[i] >= self.f_sam[j]) and (self.f_samfine[i] <= self.f_sam[j+1])):
                    self.apsd_samfine[i] = self.apsd_sam[j] * \
                        ((self.f_samfine[i] / self.f_sam[j])**self.slope[j])
                    break
