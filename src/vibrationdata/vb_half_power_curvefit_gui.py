...
class vb_half_power_curvefit:
    def __init__(self, parent):
        ...
        self.master.title(
            "vb_half_power_curvefit_gui.py  ver 1.0  by Tom Irvine")
...
###############################################################################
        ...
        self.hwtext1 = tk.Label(
            top, text='This method performs a curve-fit to determine the damping ratio for a system excited by base excitation or an applied force.')
        ...
        self.hwtext2 = tk.Label(
            top, text='This method is an extension of the half-power bandwidth method. The input data must be a frequency response function with two columns: freq(Hz) & amp')
        ...
        self.hwtext3.grid(row=crow, column=1, columnspan=1,
                          pady=15, sticky=tk.S)
        ...
        self.button_read.grid(row=crow, column=0,
                              columnspan=1, pady=1, sticky=tk.N)
        ...
        self.Lb1.grid(row=crow, column=1, columnspan=1,
                      padx=10, pady=1, sticky=tk.N)
...
#        self.Lb1.bind('<<ListboxSelect>>',self.velocity_unit_option)
        ...
        self.hwtext4.grid(row=crow, column=0, columnspan=1,
                          pady=10, sticky=tk.S)

        self.hwtext5 = tk.Label(top, text='Select Response Type')
        self.hwtext5.grid(row=crow, column=1, columnspan=1,
                          pady=10, sticky=tk.S)

        self.hwtext6 = tk.Label(top, text='Select Amplitude Dimension')
        self.hwtext6.grid(row=crow, column=2, columnspan=2,
        ...
        self.Lb2.grid(row=crow, column=0, columnspan=1,
                      padx=10, pady=3, sticky=tk.N)
        ...
        self.Lb4.grid(row=crow, column=2, columnspan=1,
                      padx=5, pady=3, sticky=tk.N)
        self.Lb4.select_set(0)

        crow = crow+1

        self.hwtext7 = tk.Label(top, text='Min Freq (Hz)')
        self.hwtext7.grid(row=crow, column=0, columnspan=1,
                          pady=10, sticky=tk.S)

        self.hwtext8 = tk.Label(top, text='Enter Number of Trials')
        self.hwtext8.grid(row=crow, column=1, columnspan=1,
                          pady=10, sticky=tk.S)

        self.hwtext10 = tk.Label(top, text='Enter Curve-fit Frequencies (Hz)')
        self.hwtext10.grid(row=crow, column=2, columnspan=2,
                           pady=10, sticky=tk.SW)
        ...
        self.ntrials_entry = tk.Entry(
            top, width=10, textvariable=self.ntrialsr)
        self.ntrials_entry.grid(row=crow, column=1)

        self.hwtext12 = tk.Label(top, text='Start')
        self.hwtext12.grid(row=crow, column=1, columnspan=2,
                           pady=10, sticky=tk.E)
        ...
        self.hwtext9.grid(row=crow, column=0, columnspan=1,
                          pady=10, sticky=tk.S)

        self.hwtext13 = tk.Label(top, text='End')
        self.hwtext13.grid(row=crow, column=2, columnspan=1,
                           pady=10, sticky=tk.E)
        ...
        self.button_plot = tk.Button(
            top, text="Plot Input FRF", command=self.plot_input)
        self.button_plot.config(height=2, width=15, state='disabled')
        self.button_plot.grid(row=crow, column=0,
                              columnspan=1, padx=5, pady=10)

        self.button_calculate = tk.Button(
            top, text="Calculate", command=self.calculate)
        self.button_calculate.config(height=2, width=15, state='disabled')
        self.button_calculate.grid(
            row=crow, column=1, columnspan=1, padx=5, pady=10)
...
#
                if (iex == 1):
                    def MFRF(omega, rho, damp): return (
                        np.sqrt((1+(2*damp*rho)**2)/((1-rho**2)**2+(2*damp*rho)**2)))
                else:
                    def MFRF(omega, rho, damp): return (
                        np.sqrt(omega**2/((1-rho**2)**2+(2*damp*rho)**2)))
...
#
                if (iex == 1):
                    ...
                    def MFRF(omega, rho, damp): return (
                        (1.+(2.*damp*rho)**2)/((1.-rho**2)**2+(2.*damp*rho)**2))
                else:
                    def MFRF(omega, rho, damp): return (
                        (omega**2/((1-rho**2)**2+(2*damp*rho)**2)))
...
#
                if (iex == 1):
                    def MFRF(omega, rho, damp): return (
                        np.sqrt(((1+(2*damp*rho)**2)/omega)/((1-rho**2)**2+(2*damp*rho)**2)))
                else:
                    def MFRF(omega, rho, damp): return (
                        np.sqrt(omega/((1-rho**2)**2+(2*damp*rho)**2)))
#
            else:
                tlabel = 'Velocity^2'
#
                if (iex == 1):
                    def MFRF(omega, rho, damp): return (
                        ((1+(2*damp*rho)**2)/omega)/((1-rho**2)**2+(2*damp*rho)**2))
                else:
                    def MFRF(omega, rho, damp): return (
                        (omega/((1-rho**2)**2+(2*damp*rho)**2)))
...
#
                if (iex == 1):
                    def MFRF(omega, rho, damp): return (
                        np.sqrt(((1+(2*damp*rho)**2)/omega**2)/((1-rho**2)**2+(2*damp*rho)**2)))
                else:
                    def MFRF(omega, rho, damp): return (
                        np.sqrt(1/((1-rho**2)**2+(2*damp*rho)**2)))
#
            else:
...
#
                if (iex == 1):
                    def MFRF(omega, rho, damp): return (
                        ((1+(2*damp*rho)**2)/omega**2)/((1-rho**2)**2+(2*damp*rho)**2))
                else:
                    def MFRF(omega, rho, damp): return (
                        (1/((1-rho**2)**2+(2*damp*rho)**2)))
        ...
        try:
            ...
            tkMessageBox.showwarning(
                "warning", "Enter start frequency", parent=self.button_calculate)
            return
        ...
        try:
            ...
            tkMessageBox.showwarning(
                "warning", "Enter end frequency", parent=self.button_calculate)
            return
