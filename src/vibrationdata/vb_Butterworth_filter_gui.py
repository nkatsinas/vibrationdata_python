...
class vb_Butterworth_filter:
    def __init__(self, parent):
        ...
        self.master.title(
            "vb_Butterworth_filter_gui.py ver 2.1  by Tom Irvine")
...
################################################################################
        ...
        self.hwtext1b = tk.Label(
            top, text='This script applies a sixth-order Butterworth filter to a time history.')
        self.hwtext1b.grid(row=crow, column=0, columnspan=6,
                           pady=7, sticky=tk.W)
...
################################################################################
        ...
        self.y_string_entry.grid(
            row=crow, column=1, columnspan=3, padx=0, pady=7, sticky=tk.W)
...
################################################################################
        ...
        self.f2_entry = tk.Entry(
            top, width=8, textvariable=self.f2r, state='disabled')
...
################################################################################
        ...
        self.button_calculate = tk.Button(
            top, text="Calculate", command=self.Butterworth_filter_main)
        ...
        self.button_transfer = tk.Button(
            top, text="Display Transfer Function", command=self.Butterworth_transfer)
...
################################################################################
        ...
        self.button_sav = tk.Button(
            top, text="Export Filtered Data", command=self.export_fd)
...
# fH=fH*fc*tpi
        ...
        if (ire == 1):
            if (typ == 1):
                out1 = ' Butterworth Lowpass Filter 6th order Refiltering fc=' + \
                    str(lpf)+' Hz'

            if (typ == 2):
                out1 = ' Butterworth Highpass Filter 6th order Refiltering fc=' + \
                    str(hpf)+' Hz'

            if (typ == 3):
                out1 = ' Butterworth Bandpass Filter 6th order Refiltering' + \
                    str(hpf)+' to '+str(lpf)+' Hz'

        else:
            if (typ == 1):
                out1 = ' Butterworth Lowpass Filter 6th order fc=' + \
                    str(lpf)+' Hz'

            if (typ == 2):
                out1 = ' Butterworth Highpass Filter 6th order fc=' + \
                    str(hpf)+' Hz'

            if (typ == 3):
                out1 = ' Butterworth Bandpass Filter 6th order' + \
                    str(hpf)+' to '+str(lpf)+' Hz'
...
################################################################################
        ...
        if (iflag == 0):
            ...
            if (n == 1):
                out1 = 'Highpass Filtered Time History fc=' + \
                    str(self.freq)+' Hz'

            if (n == 2):
                out1 = 'Bandpass Filtered Time History fc=' + \
                    str(self.fh)+' to '+str(self.fl)+' Hz'
            ...
            while (iflag == 0):
                ...
                try:
                    ...
                    # disregard error
                    plt.plot(tt, y, linewidth=1.0, color='b')
                    plt.grid(True)
                    plt.xlabel('Time (sec)')
...
################################################################################
    ...
    def read_data(self):
        ...
        self.ttime, self.y, self.num = read_two_columns_from_dialog(
        ...
        self.sr, self.dt = sample_rate_check(
            self.ttime, self.y, self.num, self.sr, self.dt)

        plt.ion()
        plt.clf()
        plt.figure(1)

        plt.plot(self.ttime, self.y, linewidth=1.0,
