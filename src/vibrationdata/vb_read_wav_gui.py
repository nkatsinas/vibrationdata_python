...
class vb_read_wav:
    ...
    def __init__(self, parent):
        ...
        self.button_read = tk.Button(
            self.top, text="Read wav File", command=self.read_wav)
        self.button_read.config(height=2, width=16)
        self.button_read.grid(row=crow, column=0,
                              columnspan=1, pady=10, sticky=tk.N)

        crow = crow+1

        self.button_play = tk.Button(
            self.top, text="Play wav File", command=self.play_wav)
        self.button_play.config(height=2, width=16, state='disabled')
        self.button_play.grid(row=crow, column=0,
                              columnspan=1, pady=10, sticky=tk.N)

        crow = crow+1

        self.button_export = tk.Button(
            self.top, text="Export Text File", command=self.export_txt)
        self.button_export.config(height=2, width=16, state='disabled')
        self.button_export.grid(
            row=crow, column=0, columnspan=1, pady=10, sticky=tk.N)

        crow = crow+1

        self.button_calculate_FFT = tk.Button(
            self.top, text="Calculate FFT", command=self.calculate_FFT)
        self.button_calculate_FFT.config(height=2, width=16, state='disabled')
        self.button_calculate_FFT.grid(
            row=crow, column=0, columnspan=1, pady=10, sticky=tk.N)
...
###############################################################################
    ...
    def read_wav(self):
        ...
        input_file_path = askopenfilename(
            parent=self.master, title=label, filetypes=[("wav files", "*.wav")])
