...
from vibrationdata.vb_utilities import read_one_column_from_dialog, read_two_columns_from_dialog
...
# self.master.geometry("800x700")
        ...
        self.hwtext1 = tk.Label(
            self.top, text='ASTM E 1049-85 (2005) Rainflow Counting Method')
        self.hwtext1.grid(row=self.crow, column=0,
                          columnspan=4, pady=7, sticky=tk.W)

        self.crow = self.crow+1

        self.hwtext2 = tk.Label(self.top, text='Select input data format ')
        self.hwtext2.grid(row=self.crow, column=0,
                          columnspan=2, pady=7, sticky=tk.SW)

        self.hwtext3 = tk.Label(self.top, text='Remove Mean ')
        self.hwtext3.grid(row=self.crow, column=2,
                          columnspan=1, padx=5, pady=7, sticky=tk.S)
        ...
        self.button_read = tk.Button(
            self.top, text="Read Input File", command=self.read_data)
        self.button_read.config(height=3, width=15)
        self.button_read.grid(row=self.crow, column=0,
                              columnspan=1, pady=10, sticky=tk.W)

        self.button_calculate = tk.Button(
            self.top, text="Calculate", command=self.calculate)
        self.button_calculate.config(height=3, width=15, state='disabled')
        self.button_calculate.grid(
            row=self.crow, column=2, columnspan=1, pady=10, sticky=tk.W)

        root = self.master

        self.button_quit = tk.Button(
            self.top, text="Quit", command=lambda root=root: quit(root))
        self.button_quit.config(height=3, width=15)
        self.button_quit.grid(row=self.crow, column=3,
                              columnspan=2, padx=10, pady=20)

        self.crow = self.crow+1

        self.tree = Treeview(self.top, selectmode="extended",
        ...
        self.hwtextext_rapv1.grid(
            row=self.crow, column=0, columnspan=1, pady=10, sticky=tk.W)

        self.hwtextext_rapv2 = tk.Label(
            self.top, text='Amplitude=(peak-valley)/2')
        self.hwtextext_rapv2.grid(
            row=self.crow, column=2, columnspan=1, pady=10, sticky=tk.W)
        ...
        self.button_rt = tk.Button(
            self.top, text="Rainflow Table (csv format)", command=self.export_table)
        self.button_rt.config(height=2, width=26, state='disabled')
        self.button_rt.grid(row=self.crow, column=0,
                            columnspan=2, pady=3, padx=1)

        self.button_rc = tk.Button(
            self.top, text="Range & Cycles", command=self.export_range_cycles)
        self.button_rc.config(height=2, width=26, state='disabled')
        self.button_rc.grid(row=self.crow, column=2,
                            columnspan=2, pady=3, padx=4)

        self.button_ac = tk.Button(
            self.top, text="Amplitude & Cycles", command=self.export_amp_cycles)
        self.button_ac.config(height=2, width=26, state='disabled')
        self.button_ac.grid(row=self.crow, column=4,
                            columnspan=2, pady=3, padx=4)
...
################################################################################
    ...
    def export_table(self):
        output_file_path = asksaveasfilename(
            parent=self.master, title="Enter the Rainflow Table Comma-Delimited filename")
        output_file = output_file_path.rstrip('\n')

        header = (['Range(units)', 'Cycle Count', 'Ave Amp',
                  'Max Amp', 'Ave Mean', 'Min Valley', 'Max Peak'])
    ...
    def export_range_cycles(self):
        output_file_path = asksaveasfilename(
            parent=self.master, title="Enter the Range & Cycles filename")
        output_file = output_file_path.rstrip('\n')
        WriteData2B(self.kv, self.B, output_file)

    def export_amp_cycles(self):
        output_file_path = asksaveasfilename(
            parent=self.master, title="Enter the Amplitude & Cycles filename")
        output_file = output_file_path.rstrip('\n')
...
# print ("i=%d  Y=%g " %(i,Y)
            ...
            for ijk in range(13, 0, -1):
                ...
                if (Y >= self.L[ijk] and Y <= self.L[ijk+1]):
                    ...
                    # weighted average
                    self.AverageMean[ijk] += self.B[i][1] * \
                        (self.B[i][3]+self.B[i][2])*0.5
