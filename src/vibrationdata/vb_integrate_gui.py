...
from vibrationdata.vb_utilities import read_two_columns_from_dialog, sample_rate_check, WriteData2

from vibrationdata.vb_utilities import BUTTERWORTH
...
class vb_integrate:
    def __init__(self, parent):
        ...
        self.hwtext2 = tk.Label(
            top, text='The input file must have two columns:  time(sec) & amplitude')
        ...
        self.hwtext2 = tk.Label(
            top, text='Select trend removal prior to integration')
        self.hwtext2.grid(row=crow, column=0, columnspan=2,
                          pady=7, sticky=tk.W)
        ...
        self.f_string.set('')
        self.f_string_entry = tk.Entry(
            top, width=10, textvariable=self.f_string)
        self.f_string_entry.grid(
            row=crow, column=2, columnspan=1, padx=20, pady=2, sticky=tk.N)
        self.f_string_entry.config(state='disabled')

        crow = crow+1

        self.hwtext71 = tk.Label(top, text='Apply Fade in?')
        self.hwtext71.grid(row=crow, column=0, columnspan=1,
                           pady=12, sticky=tk.W)

        self.hwtext72 = tk.Label(top, text='Enter percent')
        self.hwtext72.grid(row=crow, column=1, columnspan=1,
                           padx=10, pady=12, sticky=tk.W)

        self.hwtext73 = tk.Label(top, text='Apply Fade out?')
        self.hwtext73.grid(row=crow, column=3, columnspan=1,
                           pady=12, sticky=tk.W)

        self.hwtext74 = tk.Label(top, text='Enter percent')
        self.hwtext74.grid(row=crow, column=4, columnspan=1,
                           padx=10, pady=12, sticky=tk.W)
        ...
        self.fi_string_entry = tk.Entry(
            top, width=10, textvariable=self.fi_string)
        self.fi_string_entry.grid(
            row=crow, column=1, columnspan=1, padx=20, pady=1, sticky=tk.N)
        ...
        self.fo_string_entry = tk.Entry(
            top, width=10, textvariable=self.fo_string)
        self.fo_string_entry.grid(
            row=crow, column=4, columnspan=1, padx=20, pady=1, sticky=tk.N)
...
###############################################################################
        ...
        self.hwtext21.grid(row=crow, column=0, columnspan=2,
                           pady=15, sticky=tk.W)

        self.hwtext22 = tk.Label(top, text='Enter Scale Factor')
        self.hwtext22.grid(row=crow, column=2, columnspan=1,
                           padx=20, pady=15, sticky=tk.W)
        ...
        self.sc_string_entry = tk.Entry(
            top, width=10, textvariable=self.sc_string)
        self.sc_string_entry.grid(
            row=crow, column=2, columnspan=1, padx=20, pady=1, sticky=tk.N)
        ...
        self.y_string_entry.grid(
            row=crow, column=2, columnspan=2, padx=5, pady=1, sticky=tk.N)

        self.dy_string = tk.StringVar()
        self.dy_string.set('')
        self.dy_string_entry = tk.Entry(
            top, width=26, textvariable=self.dy_string)
        self.dy_string_entry.grid(
            row=crow, column=4, columnspan=2, padx=5, pady=1, sticky=tk.N)
...
###############################################################################
        ...
        self.button_sav = tk.Button(
            top, text="Export Integrated Signal", command=self.export_th)
        ...
        self.button_quit.grid(row=crow, column=4,
...
###############################################################################
    ...
    def calculation(self):
        ...
        if (imr == 2):
            ...
            self.bb = BUTTERWORTH(
                l, f, fh, fl, self.dt, iband, iphase, self.b).Butterworth_filter_main()
...
###############################################################################
        ...
        for i in range(1, int(self.num-1)):
            ...
            self.v[self.num-1] = self.v[self.num-2] + \
                self.bb[self.num-1]*self.dt/2
        ...
        plt.plot(self.a, self.v, linewidth=1.0,
                 color='b')        # disregard error
