...
###############################################################################
        ...
        self.hwtext1 = tk.Label(
            top, text='This script calculates the response of an SDOF system to an applied force PSD.')
        ...
        self.hwtext2 = tk.Label(
            top, text='The input file must have two columns: Freq(Hz) & Force(unit^2/Hz)')
        ...
        self.hwtext_e.grid(row=crow, column=2, columnspan=1,
                           padx=14, pady=10, sticky=tk.S)
        ...
        self.button_export = tk.Button(
            top, text="Export Response", command=self.export)
...
###############################################################################
    ...
    def export(self):
        ...
        if (n <= 3):
            ...
            if (n == 4):
                WriteData3(num, self.fi, self.accelerance.real,
                           self.accelerance.imag, output_file)
            if (n == 5):
                WriteData3(num, self.fi, self.mobility.real,
                           self.mobility.imag, output_file)
            if (n == 6):
                WriteData3(num, self.fi, self.receptance.real,
                           self.receptance.imag, output_file)
...
#
        ...
        if (nu == 0):
            title_string = 'Power Spectral Density   ' + \
                str("%6.3g" % rms)+' lbf RMS Overall '
            plt.ylabel(' Force (lbf^2/Hz)')
        else:
            title_string = 'Power Spectral Density   ' + \
                str("%6.3g" % rms)+' N RMS Overall '
        ...
        for i in range(0, inif):
            ...
            for j in range(0, int(m-1)):
                ...
                if ((self.fi[i] >= self.f[j]) and (self.fi[i] <= self.f[j+1]) and iflag == 0):
                    ...
                    self.ai[i] = self.a[j] * \
                        ((self.fi[i] / self.f[j])**self.slope[j])
                    iflag = 1
#            print (fi[i],ai[i])
