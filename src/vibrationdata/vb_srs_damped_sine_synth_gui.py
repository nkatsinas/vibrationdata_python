...
from vibrationdata.vb_utilities import WriteData2, WriteData3, WriteData5
from vibrationdata.vb_utilities import read_two_columns_from_dialog, SRS_function, \
...
#        self.master.geometry("1200x700")
        ...
        self.master.title(
            "vb_srs_damped_sine_synth_gui.py ver 1.0  by Tom Irvine")
        ...
        self.hwtext1 = tk.Label(
            top, text='Synthesize a time history to satisfy an SRS specification using a damped sine series with wavelet reconstruction')
        self.hwtext1.grid(row=crow, column=0, columnspan=7,
                          pady=4, sticky=tk.W)

        crow = crow+1

        self.hwtext2 = tk.Label(
            top, text='The input file must have two columns:  Natural Frequency (Hz) & SRS (G)')
        ...
        self.hwtext2 = tk.Label(
            top, text='The sample rate should be >= 10x max SRS frequency')
        self.hwtext2.grid(row=crow, column=0, columnspan=5,
                          pady=3, sticky=tk.W)

        crow = crow+1

        self.hwtext2 = tk.Label(
            top, text='The duration should be > 1/( min SRS frequency)')
        self.hwtext2.grid(row=crow, column=0, columnspan=5,
                          pady=3, sticky=tk.W)
...
###############################################################################
        ...
        self.button_read.grid(row=crow, column=0,
                              columnspan=1, pady=16, sticky=tk.W)
...
###############################################################################
        ...
        self.nwavelets_entry = tk.Entry(
            top, width=7, textvariable=self.nwaveletsr)
        self.nwavelets_entry.grid(row=crow, column=3, sticky=tk.N, pady=1)

        self.nwtrialsr = tk.StringVar()
        self.nwtrialsr.set('3000')
        self.nwtrials_entry = tk.Entry(
            top, width=7, textvariable=self.nwtrialsr)
...
###############################################################################
        ...
        self.button_calculate = tk.Button(
            top, text="Calculate", command=self.srs_synth_calculation)
...
################################################################################
        ...
        self.button_sa = tk.Button(
            top, text="Acceleration", command=self.export_a)
        ...
        self.button_sd = tk.Button(
            top, text="Displacement", command=self.export_d)
        ...
        self.button_swt = tk.Button(
            top, text="Wavelet Table", command=self.export_wt)
 ...
 #         print("ref 2a")
        ...
        for ia in range(0, ntrials):
            ...
            if (ia >= 1):
                ...
                h, m = divmod(m, 60)
                print(
                    "Remaining time = %d hr %02d min %02d sec  for phase 1" % (h, m, s))
...
#                print("ref 6")
                ...
                sym, acc2 = vb_srs_damped_sine_synth.DSS_scale_th(
                    ns, last, acc, amp, sss)
# print("*2  %ld " %(len(acc2)))
...
#               print("ref 9  %ld" % ijk)
                ...
                syn_error, iflag, db1, db2, db_total = vb_srs_damped_sine_synth.DSS_srs_error(
                    last, xmax, xmin, ra, iflag)
...
# print("*  %8.4g  %8.4g  %8.4g  %8.4g  %8.4g  %8.4g  " %(fn[k],xmax[k],xmin[k],ra[k],db2[k],db1[k]))
                ...
                print(" %ld %ld  error= %8.4g   best= %8.4g  " %
                      (ia, ijk, syn_error, errlit))
...
###############################################################################
        ...
        plt.plot(self.a, self.b, linewidth=1.0, color='k',
                 label="Spec and +/- 3dB Tol")        # disregard error
...
#        plt.plot(self.damped_sine_accel[:,0],self.damped_sine_accel[:,1], linewidth=1.0,color='g')
        ...
        syn_error, iflag, db1, db2, db_total =\
            vb_srs_damped_sine_synth.DSS_srs_error(
                last, srs_syn[:, 1], srs_syn[:, 2], ra, iflag)
...
#########################
        ...
        for ijk in range(0, 200):
            ...
            for i in range(0, NW):
                ...
                yy[index1:index2] += (x1[i]/2.) * \
                    (-np.cos(apb[index1:index2]) + np.cos(amb[index1:index2]))
...
#        print(acceleration.shape)
        ...
        plt.subplot(212)
        plt.plot(acceleration[:, 0], acceleration[:, 1],
...
##################
        ...
        plt.plot(srs_syn[:, 0], srs_syn[:, 1],
                 color='b', label="Pos Synthesis")
        plt.plot(srs_syn[:, 0], srs_syn[:, 2],
                 color='g', label="Neg Synthesis")
        plt.legend(loc="upper left")
