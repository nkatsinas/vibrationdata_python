...
def SignalAnalysisWindow():
    ...
    if (n == m):
        from vibrationdata.vb_statistics_gui import vb_statistics
        vb_statistics(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_trend_removal_scaling_gui import vb_trend_removal_scaling
        vb_trend_removal_scaling(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_various_filters_gui import vb_various_filters
        vb_various_filters(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_fourier_gui import vb_Fourier
        vb_Fourier(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_fft_gui import vb_FFT
        vb_FFT(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_waterfall_fft_gui import vb_waterfall_FFT
        vb_waterfall_FFT(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_psd_gui import vb_PSD
        vb_PSD(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_sdof_response_time_domain_gui import vb_sdof_response_time_domain
        vb_sdof_response_time_domain(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_srs_gui import vb_SRS
        vb_SRS(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_rainflow_gui import vb_rainflow
        vb_rainflow(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_integrate_differentiate_gui import vb_integrate_differentiate
        vb_integrate_differentiate(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_autocorrelation_gui import vb_autocorrelation
        vb_autocorrelation(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_cross_correlation_gui import vb_cross_correlation
        vb_cross_correlation(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_cepstrum_gui import vb_cepstrum
        vb_cepstrum(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_spl_gui import vb_SPL
        vb_SPL(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_sine_curvefit_gui import vb_sine_curvefit
        vb_sine_curvefit(win)
    m = m+1

    if (n == m):
        from vibrationdata.vb_tvfa_gui import vb_tvfa
        vb_tvfa(win)
    m = m+1
...
def PSDAnalysisWindow():
    ...
    if (n == m):
        from vibrationdata.vb_psd_rms_gui import vb_psd_rms
        vb_psd_rms(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_psd_octave_gui import vb_psd_octave
        vb_psd_octave(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_sdof_base_psd_gui import vb_sdof_base_psd
        vb_sdof_base_psd(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_sdof_force_psd_gui import vb_sdof_force_psd
        vb_sdof_force_psd(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_vrs_gui import vb_VRS
        vb_VRS(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_vrs_force_gui import vb_VRS_force
        vb_VRS_force(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_accel_psd_syn_gui import vb_accel_psd_syn
    ...
    if (n == m):
        ...
        from vibrationdata.vb_force_psd_syn_gui import vb_force_psd_syn
        vb_force_psd_syn(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_power_trans_gui import vb_power_trans
        vb_power_trans(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_envelope_PSD_VRS_gui import vb_envelope_PSD_VRS
...
def SRSAnalysisWindow():
    ...
    if (n == m):
        from vibrationdata.vb_srs_damped_sine_synth_gui import vb_srs_damped_sine_synth
...
#        from vibrationdata.vb_srs_wavelet_synth_gui import vb_srs_wavelet_synth
...
def MiscAnalysisWindow():
    ...
    if (n == m):
        from vibrationdata.vb_amplitude_conversion_gui import vb_amplitude_conversion
        vb_amplitude_conversion(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_sine_sweep_parameters_gui import vb_sine_sweep_parameters
        vb_sine_sweep_parameters(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_peak_sigma_random_gui import vb_peak_sigma_random
        vb_peak_sigma_random(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_classical_base_gui import vb_classical_pulse_base
        vb_classical_pulse_base(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_steady_gui import vb_steady
        vb_steady(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_generate_gui import vb_generate
        vb_generate(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_structural_dynamics_gui import vb_structural_dynamics
        vb_structural_dynamics(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_plot_utilities_gui import vb_plot_utilities
        vb_plot_utilities(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_signal_editing_gui import vb_signal_editing
        vb_signal_editing(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_rotation_gui import vb_rotation
        vb_rotation(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_miles_gui import vb_miles
        vb_miles(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_statistical_distributions_gui import vb_statistical_distributions
        vb_statistical_distributions(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_Doppler_shift_gui import vb_Doppler_shift
        vb_Doppler_shift(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_dB_calculations_gui import vb_dB_calculations
        vb_dB_calculations(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_sound_file_editor_gui import vb_sound_file_editor
        vb_sound_file_editor(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_modal_frf_gui import vb_modal_frf
        vb_modal_frf(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_damping_utilities_gui import vb_damping_utilities
        vb_damping_utilities(win)

    m = m+1

    if (n == m):
        from vibrationdata.vb_wind_waves_gui import vb_wind_waves
        vb_wind_waves(win)
...
w = int(2.*(w*0.37))  # do not change
h = int(2.*(h*0.32))  # do not change
root.geometry("%dx%d+0+0" % (w, h))


root.title("vibrationdata.py ver 6.7  by Tom Irvine")
...
hwtext1 = tk.Label(
    root, text='Note:  for use within Spyder IDE, set: Run > Configuration > Interpreter > Excecute in an external system terminal')
...
button1 = tk.Button(root, text='Perform Signal Analysis',
                    command=SignalAnalysisWindow)
button1.grid(row=crow, column=0, padx=8, pady=10)
button1.config(height=2, width=30)

button3 = tk.Button(root, text='Perform PSD Analysis',
                    command=PSDAnalysisWindow)
button3.grid(row=crow, column=1, padx=8, pady=10)
button3.config(height=2, width=20)

button4 = tk.Button(root, text='Perform SRS Analysis',
                    command=SRSAnalysisWindow)
button4.grid(row=crow, column=2, padx=8, pady=10)
button4.config(height=2, width=20)

button2 = tk.Button(
    root, text='Perform Miscellaneous Analysis', command=MiscAnalysisWindow)
...
button_quit = tk.Button(
    root, text="Quit", command=lambda root=root: quit(root))
