...
###############################################################################
        ...
        self.hwtextfn.grid(row=crow, column=0, columnspan=2,
                           padx=15, sticky=tk.W)
...
###############################################################################
    ...
    def export_rd(self):
        root = self.master
        output_file_path = asksaveasfilename(
            parent=root, title="Enter the relative displacement filename")
        output_file = output_file_path.rstrip('\n')
    ...
    def rd_arbit_base(self):
        ...
        plt.plot(self.a, self.rd_resp, linewidth=1.0,
        ...
        out1 = 'SDOF Response Time History  fn=%g Hz, Q=%g ' % (
            self.fn, self.Q)
