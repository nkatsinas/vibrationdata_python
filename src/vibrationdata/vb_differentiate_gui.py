...
###############################################################################
        ...
        self.hwtext1t.grid(row=crow, column=0, columnspan=2,
                           pady=7, sticky=tk.W)
...
################################################################################
        ...
        self.hwtext3b = tk.Label(
            top, text='Enter Output Time History Y-axis Label')
        self.hwtext3b.grid(row=crow, column=0, columnspan=2,
                           pady=11, sticky=tk.E)
...
###############################################################################
        ...
        self.scale_string_entry = tk.Entry(
            top, width=26, textvariable=self.scale_string)
        self.scale_string_entry.grid(
            row=crow, column=2, columnspan=1, padx=5, pady=4, sticky=tk.N)
...
################################################################################
        ...
        self.button_calculate = \
            tk.Button(top, text="Calculate",
                      command=self.differentiate_calculation)
...
################################################################################
        ...
        self.button_differentiated = \
            tk.Button(top, text="Export Differentiated Time History",
                      command=self.export_differentiated)
        self.button_differentiated.config(height=2, width=30, state='disabled')
        self.button_differentiated.grid(
            row=crow, column=0, columnspan=2, pady=1, padx=1)
...
################################################################################
    ...
    def differentiate_calculation(self):
        ...
        for i in range(2, int(self.num-2)):
            self.v[i] = (-self.b[i+2] + 8.*self.b[i+1] -
                         8.*self.b[i-1] + self.b[i-2]) / ddt

        self.v[self.num-2] = (self.b[self.num-2] -
                              self.b[self.num-4])/(2.*self.dt)
