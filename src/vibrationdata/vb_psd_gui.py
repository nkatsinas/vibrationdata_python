...
from vibrationdata.vb_utilities import read_two_columns_from_dialog, sample_rate_check
...
################################################################################
        ...
        self.button_read.grid(row=crow, column=0,
                              columnspan=1, pady=9, sticky=tk.W)
...
################################################################################
        ...
        self.button_calculate = tk.Button(
            top, text="Calculate PSD", command=self.psd_calculation)
        self.button_calculate.config(height=2, width=15, state='disabled')
        self.button_calculate.grid(row=crow, column=0, pady=20)

        self.button_replot = tk.Button(
            top, text="Replot PSD", command=self.replot)
        ...
        self.button_octave = tk.Button(
            top, text="Octave Format", command=self.octave)
...
#        tk_PSD.psd_plots
    ...
    def psd_plots(cls, self):
        ...
        try:
            ...
            tkMessageBox.showwarning(
                "Warning", "Enter Min Frequency", parent=self.button_calculate)
            return
        ...
        try:
            ...
            tkMessageBox.showwarning(
                "Warning", "Enter Max Frequency", parent=self.button_calculate)
            return
