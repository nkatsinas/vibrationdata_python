...
from vibrationdata.vb_utilities import sample_rate_check, read_two_columns_from_dialog, WriteData2
...
###############################################################################
        ...
        self.hwtext0 = tk.Label(
            top, text='This script calculates sound pressure level of a time history.')
        self.hwtext0.grid(row=crow, column=0, columnspan=2,
                          pady=6, sticky=tk.NW)

        crow = crow+1

        self.hwtext3 = tk.Label(
            top, text='The input file must have two columns: time(sec) & pressure')
...
###############################################################################
        ...
        self.button_read.grid(row=crow, column=0,
                              columnspan=1, pady=9, sticky=tk.NW)
        ...
        self.hwtext3b.grid(row=crow, column=0, columnspan=1,
                           pady=7, sticky=tk.S)
...
################################################################################
        ...
        self.tmir.set('')
        self.tmi_entry = tk.Entry(
            top, width=8, textvariable=self.tmir, state='disabled')
        ...
        self.tmer.set('')
        self.tme_entry = tk.Entry(
            top, width=8, textvariable=self.tmer, state='disabled')
...
###############################################################################
        ...
        self.button_process = tk.Button(
            top, text="Show Processing Options", command=self.processing_options)
...
###############################################################################
        ...
        self.button_save = tk.Button(
            top, text="Save SPL", command=self.export_spl)
...
###############################################################################
    ...
    def export_spl(self):
        output_file_path = asksaveasfilename(
            parent=self.master, title="Enter the SPL filename")
...
###############################################################################
    ...
    def calculation(self):
        ...
        mk, freq, time_a, dt, NW = vb_SPL.FFT_time_freq_set(
            mmm, NW, self.dt, df, tmi, io)

        store, store_p, freq_p, max_a, max_f = vb_SPL.FFT_core_seg(
            NW, mmm, mk, freq, self.amp, io)
...
#        print (" store  imax=%d  jmax=%d  " %(imax,jmax))
        ...
        splevel, oaspl = vb_SPL.convert_one_third_octave_mag_to_dB(
            band_rms, ref)
        ...
        if (nw == 0):
            ...
            self.pf, self.pspl, oaspl, scale_label = vb_SPL.acoustic_weighting(
                self.pf, self.pspl, imax, ref, nw)
        ...
        if (nw == 0):
            ...
            out7 = 'One-Third Octave SPL %s-scale  OASPL=%7.4g dB  Ref= 20 micro Pa' % (
                scale_label, oaspl)
