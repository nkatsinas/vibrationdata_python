...
class vb_generate:
    def __init__(self, parent):
        ...
        self.button_quit.grid(row=crow, column=1, padx=10,
                              pady=15, sticky=tk.SW)
...
################################################################################
    ...
    def calculation(self):
        ...
        if (n == 0):
            from vibrationdata.vb_generate_sine_gui import vb_generate_sine
            vb_generate_sine(win)

        if (n == 1):
            from vibrationdata.vb_generate_cosine_gui import vb_generate_cosine
            vb_generate_cosine(win)

        if (n == 2):
            from vibrationdata.vb_damped_sine_gui import vb_damped_sine
            vb_damped_sine(win)

        if (n == 3):
            from vibrationdata.vb_white_noise_gui import vb_white_noise
            vb_white_noise(win)

        if (n == 4):
            from vibrationdata.vb_pink_noise_gui import vb_pink_noise
            vb_pink_noise(win)

        if (n == 5):
            from vibrationdata.vb_sine_sweep_gui import vb_sine_sweep
            vb_sine_sweep(win)
