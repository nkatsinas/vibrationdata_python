...
###############################################################################
        ...
        self.hwtext3 = tk.Label(
            top, text='This script converts a PSD into octave format')
        ...
        self.hwtext3 = tk.Label(
            top, text='The input file must have two columns: Freq(Hz) & PSD(unit^2/Hz)')
        self.hwtext3.grid(row=crow, column=0, columnspan=2,
                          pady=6, sticky=tk.W)
...
###############################################################################
        ...
        self.button_save = tk.Button(
            top, text="Save PSD", command=self.export_psd)
...
#
        ...
        title_string = 'Power Spectral Density   ' + \
            str("%6.3g %s" % (rms, self.s1))+' RMS Overall '
...
#
            if s < -1.0001 or s > -0.9999:
                rb += (self.ossum[i+1] * self.ff[i+1] -
                       self.ossum[i]*self.ff[i])/(s+1.)
            else:
...
###############################################################################
        ...
        if (nb == 0):
            title_string = 'Full Octave PSD ' + \
                str("%6.3g %s" % (rms2, self.s1))+' RMS Overall '
        if (nb == 1):
            title_string = 'One-Third Octave PSD ' + \
                str("%6.3g %s" % (rms2, self.s1))+' RMS Overall '
        if (nb == 2):
            title_string = 'One-Sixth Octave PSD ' + \
                str("%6.3g %s" % (rms2, self.s1))+' RMS Overall '
        if (nb == 3):
            title_string = 'One-Twelfth Octave PSD ' + \
                str("%6.3g %s" % (rms2, self.s1))+' RMS Overall '
...
###############################################################################
    ...
    def export_psd(self):
        output_file_path = asksaveasfilename(
            parent=self.master, title="Enter the PSD filename")
