...
################################################################################
        ...
        self.hwtext1b = tk.Label(
            top, text='This script performs a sine or damped sine curve-fit for a time history.')
...
################################################################################
        ...
        self.num_sines_entry = tk.Entry(
            top, width=12, textvariable=self.num_sines)
        self.num_sines_entry.grid(
            row=crow, column=1, padx=10, pady=1, sticky=tk.N)
        ...
        self.ntrials_entry.grid(
            row=crow, column=2, padx=10, pady=1, sticky=tk.N)
...
################################################################################
        ...
        self.button_calculate.grid(
            row=crow, column=1, padx=10, pady=10, sticky=tk.S)
        ...
        if (self.ntype == 0):
            ...
            for ie in range(0, self.nfr):
                ...
                for i in range(0, self.n):
                    ...
                    self.syn[i] = self.syn[i]+self.x1r[self.ie] * \
                        np.sin(self.x2r[self.ie]*tt-self.x3r[self.ie])
            ...
            for self.ie in range(0, self.nfr):
                ...
                for i in range(0, self.n):
                    ...
                    if (tt > self.x5r[self.ie]):
                        ...
                        self.syn[i] = self.syn[i]+self.x1r[self.ie] * \
                            np.exp(-domegan*ta) * \
                            np.sin(omegad*ta-self.x3r[self.ie])
    ...
    def sine_curvefit_main(cls, self):
        ...
        for ie in range(0, self.nfr):
            ...
            print(' %d    %9.4f    %9.4f  %9.4f' % (
                m, self.x1r[self.ie], self.x2r[self.ie]/self.tp, self.x3r[self.ie]))
    ...
    def sfa_engine(cls, self):
        ...
        for j in range(0, self.nt):
            ...
            if (np.std(error) < errormax):
                ...
                print(' %6ld  %13.4e  %10.4g %9.4f %9.4f  ' %
                      (j, errormax, x1, x2/self.tp, x3))
