...
from vibrationdata.vb_utilities import read_two_columns_from_dialog, read_three_columns_from_dialog, WriteData2
...
###############################################################################
        ...
        self.hwtext3 = tk.Label(
            top, text='This script calculates the FRF from a force and response time history pair.')
        ...
        self.button_read.grid(row=crow, column=0,
                              columnspan=1, padx=0, pady=8, sticky=tk.N)

        crow = crow+1

        self.button_read = tk.Button(
            top, text="Go to Analysis Page", command=self.analysis)
        self.button_read.config(height=2, width=25)
        self.button_read.grid(row=crow, column=0,
                              columnspan=1, padx=0, pady=8, sticky=tk.N)
...
###############################################################################
    ...
    def analysis(self):
        ...
        if (self.analysis_type == 0):  # ensemble
            from vibrationdata.vb_modal_ensemble_gui import vb_modal_ensemble_frf
            vb_modal_ensemble_frf(win, self.t, self.c)

        else:                  # single record
            from vibrationdata.vb_modal_single_frf_gui import vb_modal_single_frf
    ...
    def read_data(self):
        ...
        if (self.iunit == 0):  # common array
            self.t, self.a, self.b, self.num = read_three_columns_from_dialog(
            ...
            if (La != Lb):
                tkMessageBox.showinfo(
                    "Warning", " Vector length error", parent=self.button_read_data)
                return

            if (abs(self.t[0]-self.tb[0]) > 0.0005):
                tkMessageBox.showinfo(
                    "Warning", " Vector synch error", parent=self.button_read_data)
                return
