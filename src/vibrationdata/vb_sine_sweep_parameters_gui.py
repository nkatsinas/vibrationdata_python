...
class vb_sine_sweep_parameters:
    def __init__(self, parent):
        ...
        self.master.title(
            "vb_sine_sweep_parameters_gui.py  ver 1.0  by Tom Irvine")
...
###############################################################################
    ...
    def analysis_go(self):
        ...
        if (n == m):
            from vibrationdata.vb_crossover_frequency_gui import vb_crossover_frequency
            vb_crossover_frequency(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_number_octaves_gui import vb_number_octaves
            vb_number_octaves(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_sweep_rate_gui import vb_sweep_rate
            vb_sweep_rate(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_total_sine_cycles_gui import vb_total_sine_cycles
