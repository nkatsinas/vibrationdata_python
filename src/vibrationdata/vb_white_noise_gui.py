...
from vibrationdata.vb_utilities import WriteData2, signal_stats
...
class vb_white_noise:
    ...
    def __init__(self, parent):
        ...
        self.hwtext7.grid(row=crow, column=1, columnspan=1,
                          pady=10, sticky=tk.SW)
        ...
        self.hwtext25.grid(row=crow, column=1, columnspan=1,
                           pady=10, sticky=tk.SW)
    ...
    def calculation(self):
        ...
        if not self.ampr.get():
            tkMessageBox.showwarning(
                'Warning', 'Enter std dev', parent=self.button_calculate)
            return

        if not self.srr.get():
            tkMessageBox.showwarning(
                'Warning', 'Enter sample rate', parent=self.button_calculate)
            return

        if not self.durr.get():
            tkMessageBox.showwarning(
                'Warning', 'Enter duration', parent=self.button_calculate)
            return
        ...
        if (n == 0):
            ...
            self.a = vb_white_noise.Butterworth_filter(
                self.TT, self.a, self.np, self.lpf, dt)
