...
class vb_arbit_force:
    def __init__(self, parent):
        ...
        self.hwtext2 = tk.Label(
            top, text='The input file must have two columns:  time(sec) & Force')
...
###############################################################################
        ...
        self.button_read.grid(row=crow, column=0,
                              columnspan=1, pady=20, sticky=tk.W)
...
###############################################################################
        ...
        self.button_sv = tk.Button(
            top, text="Velocity", command=self.export_vel)
        ...
        self.button_sd = tk.Button(
            top, text="Displacement", command=self.export_disp)
        self.button_sd.config(height=2, width=15, state='disabled')
        self.button_sd.grid(row=crow, column=2, columnspan=1, pady=3, padx=1)

        self.button_sft = tk.Button(
            top, text="Transmitted Force", command=self.export_trans_force)
...
#
        ...
        plt.plot(self.a, self.a_resp, linewidth=1.0,
        ...
        if (n1 == 0):
            plt.ylabel('Accel (G)')
            print("\n Accel (G)")
            out1 = 'SDOF Acceleration Response  mass=%g lbm, fn=%g Hz, Q=%g ' % (
                self.mass_orig, self.fn, self.Q)
        else:
            plt.ylabel('Accel (m/sec^2)')
            print("\n Accel (m/sec^2)")
            out1 = 'SDOF Acceleration Response  mass=%g kg, fn=%g Hz, Q=%g ' % (
                self.mass_orig, self.fn, self.Q)

        print("\n  max = %8.4g   min = %8.4g   std dev = %8.4g" %
              (a_pos, a_neg, a_sd))
...
#
        ...
        plt.plot(self.a, self.v_resp, linewidth=1.0,
                 color='b')        # disregard error
        ...
        if (n1 == 0):
            ...
            print("\n Vel (in/sec)")
            out1 = 'SDOF Velocity Response  mass=%g lbm, fn=%g Hz, Q=%g ' % (
                self.mass_orig, self.fn, self.Q)
        else:
            plt.ylabel('Vel (m/sec)')
            print("\n Vel (m/sec)")
            out1 = 'SDOF Velocity Response  mass=%g kg, fn=%g Hz, Q=%g ' % (
                self.mass_orig, self.fn, self.Q)
...
#
        ...
        plt.plot(self.a, self.d_resp, linewidth=1.0,
                 color='b')        # disregard error
        ...
        if (n1 == 0):
            plt.ylabel('Disp (in)')
            print("\n Disp (in)")
            out1 = 'SDOF Displacement Response  mass=%g lbm, fn=%g Hz, Q=%g ' % (
                self.mass_orig, self.fn, self.Q)
        else:
            plt.ylabel('Disp (mm)')
            print("\n Disp (mm)")
            out1 = 'SDOF Displacement Response  mass=%g kg, fn=%g Hz, Q=%g ' % (
                self.mass_orig, self.fn, self.Q)
    ...
    def trans_force(cls, self):
        ...
        plt.plot(self.a, self.ft_resp, linewidth=1.0,
                 color='b')        # disregard error
        ...
        if (n1 == 0):
            ...
            out1 = 'SDOF Transmitted Force  mass=%g lbm, fn=%g Hz, Q=%g ' % (
                self.mass_orig, self.fn, self.Q)
        else:
            plt.ylabel('Force (N)')
            print("\n Transmitted Force (N)")
            out1 = 'SDOF Transmitted Force  mass=%g kg, fn=%g Hz, Q=%g ' % (
                self.mass_orig, self.fn, self.Q)
...
###############################################################################
    ...
    def d_coeff(cls, self):
        ...
        self.df[1] = -2*self.omeganT*self.ecosd + 2 * \
            self.damp*(1-self.eee2) - DD2*self.esind
        self.df[2] = (2*self.damp+self.omeganT)*self.eee2 + \
            (DD1*self.esind-2*self.damp*self.ecosd)
...
###############################################################################
    ...
    def export_accel(self):
        ...
        output_file_path = asksaveasfilename(
            parent=root, title="Enter the acceleration filename")
    ...
    def export_vel(self):
        root = self.master
        output_file_path = asksaveasfilename(
            parent=root, title="Enter the velocity filename")
        output_file = output_file_path.rstrip('\n')
    ...
    def export_disp(self):
        root = self.master
        output_file_path = asksaveasfilename(
            parent=root, title="Enter the displacement filename")
        output_file = output_file_path.rstrip('\n')
    ...
    def export_trans_force(self):
        root = self.master
        output_file_path = asksaveasfilename(
            parent=root, title="Enter the transmitted force filename")
        output_file = output_file_path.rstrip('\n')
