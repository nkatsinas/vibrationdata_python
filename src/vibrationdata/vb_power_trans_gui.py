...
################################################################################
        ...
        self.hwtext1 = tk.Label(
            top, text='Each input file must have two columns:  freq(Hz) & PSD (unit^2/Hz) ')
...
################################################################################
        ...
        self.hwtext91.grid(row=crow, column=0, columnspan=1,
                           pady=3, sticky=tk.S)

        self.hwtext97 = tk.Label(top, text='Ymin')
        self.hwtext97.grid(row=crow, column=1, columnspan=1,
                           pady=3, sticky=tk.S)
        ...
        self.xmin_string_entry.grid(
            row=crow, column=0, columnspan=1, padx=5, pady=1, sticky=tk.N)
        ...
        self.hwtext92.grid(row=crow, column=0, columnspan=1,
                           pady=0, sticky=tk.S)

        self.hwtext99 = tk.Label(top, text='Ymax')
        self.hwtext99.grid(row=crow, column=1, columnspan=1,
                           pady=0, sticky=tk.S)
...
###############################################################################
        ...
        self.button_read1 = tk.Button(
            top, text="Read Response PSD File", command=self.read_data1)
        self.button_read1.config(height=2, width=25)
        self.button_read1.grid(
            row=crow, column=0, columnspan=1, padx=4, pady=10, sticky=tk.W)

        self.button_read2 = tk.Button(
            top, text="Read Input PSD File", command=self.read_data2)
        self.button_read2.config(height=2, width=25)
        self.button_read2.grid(
            row=crow, column=1, columnspan=1, padx=4, pady=10, sticky=tk.W)

        crow = crow+1

        self.button_plot12 = tk.Button(
            top, text="Plot Transmissibility", command=self.plot12)
...
################################################################################
    ...
    def read_data1(self):
        ...
        self.a1, self.b1, self.num1 = read_two_columns_from_dialog(
            'Select Response PSD File', self.master)
    ...
    def read_data2(self):
        ...
        self.a2, self.b2, self.num2 = read_two_columns_from_dialog(
            'Select Input PSD File', self.master)
    ...
    def plot12(self):
        ...
        if (num1 == 0):
            tkMessageBox.showwarning(
                "Warning", "arrays have zero length", parent=self.button_read1)
        ...
        for i in range(0, num):
            ...
            if (self.a1[i] >= 1.0e-08 and self.a1[i] <= 1.0e+08):
                ...
                if (diffr > 0.02):
                    tkMessageBox.showwarning(
                        "Warning", "frequency sync error", parent=self.button_plot12)
