...
from vibrationdata.vb_utilities import BUTTERWORTH, integrate_th, half_cosine_fade_perc
...
class vb_double_integrate:
    def __init__(self, parent):
        ...
        self.hwtext1 = tk.Label(
            top, text='This script integrates an acceleration time history to velocity and displacement.')
        ...
        self.hwtext5.grid(row=crow, column=2, columnspan=1,
                          pady=7, sticky=tk.S)

        self.hwtext6 = tk.Label(top, text='Displacement Processing')
        self.hwtext6.grid(row=crow, column=3, columnspan=1,
                          pady=7, sticky=tk.S)
        ...
        self.Lba.grid(row=crow, column=1, columnspan=1,
                      pady=1, padx=6, sticky=tk.N)
        ...
        self.Lbv.grid(row=crow, column=2, columnspan=1,
                      pady=1, padx=6, sticky=tk.N)
        ...
        self.Lbd.grid(row=crow, column=3, columnspan=1,
                      pady=1, padx=6, sticky=tk.N)
...
###############################################################################
        ...
        self.f_string_entry.grid(
            row=crow, column=0, columnspan=1, padx=10, pady=2, sticky=tk.N)
        self.f_string_entry.config(state='disabled')

        self.fv_string = tk.StringVar()
        self.fv_string.set('0')
        self.fv_string_entry = tk.Entry(
            top, width=10, textvariable=self.fv_string)
        self.fv_string_entry.grid(
            row=crow, column=2, columnspan=1, padx=10, pady=2, sticky=tk.N)

        self.fd_string = tk.StringVar()
        self.fd_string.set('0')
        self.fd_string_entry = tk.Entry(
            top, width=10, textvariable=self.fd_string)
        self.fd_string_entry.grid(
            row=crow, column=3, columnspan=1, padx=10, pady=2, sticky=tk.N)
...
###############################################################################
        ...
        self.button_sa = tk.Button(
            top, text="Save Acceleration", command=self.save_accel)
        self.button_sa.config(height=2, width=20, state='disabled')
        self.button_sa.grid(row=crow, column=0, columnspan=1, pady=18, padx=5)

        self.button_sv = tk.Button(
            top, text="Save Velocity", command=self.save_velox)
        ...
        self.button_sd = tk.Button(
            top, text="Save Displacement", command=self.save_disp)
...
###########
        ...
        if (na == 5):  # remove third order
            ...
            for i in range(0, self.num-1):
                cu = (acc[0]*(self.t[i])**3 + acc[1] *
                      (self.t[i])**2+acc[2]*(self.t[i])+acc[3])
...
########
        ...
        if (nv == 5):  # remove third order
            ...
            for i in range(0, self.num-1):
                cu = (vcc[0]*(self.t[i])**3 + vcc[1] *
                      (self.t[i])**2+vcc[2]*(self.t[i])+vcc[3])
...
#########
        ...
        if (nd == 5):  # remove third order
            ...
            for i in range(0, self.num-1):
                cu = (dcc[0]*(self.t[i])**3 + dcc[1] *
                      (self.t[i])**2+dcc[2]*(self.t[i])+dcc[3])
...
###############################################################################
    ...
    def read_data(self):
        ...
        self.sr, self.dt = sample_rate_check(
            self.t, self.b, self.num, self.sr, self.dt)
