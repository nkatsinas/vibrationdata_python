...
class vb_plot_large_data_array:
    def __init__(self, parent, TT, a, xl, yl, ti, fig_num):
        ...
        self.master.title(
            "vb_plot_large_data_array.py  ver 1.0  by Tom Irvine")
        ...
        self.hwtext4 = tk.Label(
            top, text='Plotting option for arrays > 500K coordinates')
        self.hwtext4.grid(row=crow, column=0, pady=7)

        crow = crow+1

        self.hwtext5 = tk.Label(
            top, text='Plot options affect data appearance only.')
        self.hwtext5.grid(row=crow, column=0, pady=7)

        crow = crow+1

        self.hwtext6 = tk.Label(
            top, text='The raw data array remains unmodified.')
        self.hwtext6.grid(row=crow, column=0, pady=7)

        crow = crow+1

        self.button_plot1 = tk.Button(
            top, text="Plot with Markers Only", command=self.calculate_plot1)
        self.button_plot1.config(height=2, width=25)
        self.button_plot1.grid(
            row=crow, column=0, padx=2, pady=10, sticky=tk.S)

        crow = crow+1

        self.button_plot2 = tk.Button(
            top, text="Plot Downsample/Decimate", command=self.calculate_plot2)
        ...
        self.button_plot3 = tk.Button(
            top, text="Plot Max & Min per Segment", command=self.calculate_plot3)
        self.button_plot3.config(height=2, width=25)
        self.button_plot3.grid(
            row=crow, column=0, padx=2, pady=10, sticky=tk.S)
