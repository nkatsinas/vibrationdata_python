...
class vb_statistical_distributions:
    ...
    def __init__(self, parent):
        ...
        self.master.title(
            "vb_statistical_distributions_gui.py ver 1.0  by Tom Irvine")
...
################################################################################
    ...
    def PerformAnalysis(self):
        ...
        if (n == m):
            from vibrationdata.vb_normal_gui import vb_normal
            vb_normal(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_Rayleigh_gui import vb_Rayleigh
            vb_Rayleigh(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_Poisson_gui import vb_Poisson
            vb_Poisson(win)
