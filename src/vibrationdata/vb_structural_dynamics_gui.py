...
class vb_structural_dynamics:
    ...
    def __init__(self, parent):
        ...
        self.master.title(
            "vb_structural_dynamics_gui.py ver 1.1  by Tom Irvine")
...
################################################################################
    ...
    def PerformAnalysis(self):
        ...
        if (n == m):
            from vibrationdata.vb_sdof_fn_gui import vb_sdof_fn
            vb_sdof_fn(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_beam_bending_gui import vb_beam_bending
            vb_beam_bending(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_rod_longitudinal_gui import vb_rod_longitudinal
            vb_rod_longitudinal(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_rectangular_plate_bending_gui import vb_rectangular_plate_bending
            vb_rectangular_plate_bending(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_circular_plate_bending_gui import vb_circular_plate_bending
            vb_circular_plate_bending(win)

        m = m+1

        if (n == m):
            from vibrationdata.vb_annular_plate_bending_gui import vb_annular_plate_bending
