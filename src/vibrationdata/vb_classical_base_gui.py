...
from vibrationdata.vb_utilities import WriteData2, WriteData3
...
class vb_classical_pulse_base:
    ...
    def __init__(self, parent):
        ...
        self.hwtext1 = tk.Label(
            top, text='SDOF Response to Classical Base Input')
        ...
        self.hwtext1.grid(row=crow, column=0, columnspan=1,
                          pady=5, sticky=tk.SW)
        ...
        self.hwtext3.grid(row=crow, column=0, columnspan=1,
                          pady=5, sticky=tk.SW)

        self.hwtext_base_accel = tk.Label(top, text='Enter Acceleration (G)')
        self.hwtext_base_accel.grid(
            row=crow, column=1, columnspan=1, padx=18, pady=5, sticky=tk.S)

        self.hwtext_fn = tk.Label(top, text='Enter Duration (sec)')
        self.hwtext_fn.grid(row=crow, column=2, columnspan=1,
                            padx=18, pady=5, sticky=tk.S)
        ...
        self.Lb_pulse.grid(row=crow, column=0, columnspan=1,
                           pady=1, sticky=tk.NW)
...
###############################################################################
        ...
        self.hwtext_fn.grid(row=crow, column=0, columnspan=1,
                            padx=14, pady=10, sticky=tk.S)

        self.hwtext_Q = tk.Label(top, text='Enter Q')
        self.hwtext_Q.grid(row=crow, column=1, columnspan=1,
                           padx=14, pady=10, sticky=tk.S)
        ...
        self.hwtext_start_freq = tk.Label(
            top, text='Enter Starting Frequency (Hz)')
        self.hwtext_start_freq.grid(
            row=crow, column=0, columnspan=1, padx=14, pady=15)
        self.hwtext_start_freq.config(state='disabled')

        self.hwtext_end_freq = tk.Label(
            top, text='Enter Ending Frequency (Hz)')
        self.hwtext_end_freq.grid(
            row=crow, column=1, columnspan=1, padx=14, pady=15)
        ...
        self.fstart_entry.grid(
            row=crow, column=0, padx=18, pady=1, sticky=tk.N)
        ...
        self.button_calculate = tk.Button(
            top, text="Calculate", command=self.calculate_main)
        ...
        self.button_quit.grid(row=crow, column=1,
                              columnspan=1, padx=10, pady=16)
        ...
        self.hwtext_export.grid(
            row=crow, column=0, columnspan=2, padx=10, pady=10)
        self.hwtext_export.config(state='disabled')

        crow = crow+1

        self.button_acceleration = tk.Button(
            top, text="Acceleration", command=self.export_acceleration)
        ...
        self.button_rel_disp = tk.Button(
            top, text="Rel Disp", command=self.export_rel_disp)
...
################################################################################
    ...
    def export_rel_disp(self):
        ...
        if (self.n_at == 0):
            ...
            nf = len(self.ex_fn)
            WriteData3(nf, self.ex_fn, self.ex_rdmax,
                       self.ex_rdmin, output_file)
        ...
        if (self.n_at == 0):  # time history
            ...
            if len(f_string) == 0:
                tkMessageBox.showerror(
                    'Warning:', 'Enter natural frequency', parent=self.button_calculate)
...
##
        ...
        if (self.n_at == 0):
            ...
            print(" ")
            print(" Acceleration ")
            print("    Max= %8.4g G   Min= %8.4g G " %
                  (self.amax[0], self.amin[0]))
            print(" ")
            print(" Relative Displacement ")

            if (self.n_scale == 0):
                print("    Max= %8.4g in   Min= %8.4g in " %
                      (self.rdmax[0], self.rdmin[0]))
            else:
                print("    Max= %8.4g mm   Min= %8.4g mm " %
                      (self.rdmax[0], self.rdmin[0]))
...
####
    ...
    def classical_plot_th(cls, Q, fff, tt, acc, yb, rd, n_scale):
        ...
        title_string_acc = 'Acceleration SDOF Response fn= ' + \
            fff + ' Hz   Q='+str(Q)
        title_string_rd = 'Rel Disp  SDOF Response fn= ' + \
            fff + ' Hz   Q='+str(Q)
#
...
################################################################################
    ...
    def calculate_half_sine(cls, self):
        ...
        for k in range(0, self.numf):
            ...
            a1 = self.rd_initial
            a2 = (self.rv_initial+self.domegan[k]
                  * self.rd_initial)/self.omegad[k]
...
#  Find residual parameters
            ...
            for i in range(0, self.nt[k]):
                ...
                if t <= self.dur:
                    ...
                    rd3 = - \
                        ((self.amp/self.den[k])*ee*self.omega /
                         self.omegad[k])*(c1*cwdt + c2*swdt)
                    ...
                    rv2 = self.omega * \
                        (self.amp/self.den[k]) * (-b1*sat + b2*cat)
                    rv3 = -self.domegan[k]*rd3
                    rv3 += -((self.amp/self.den[k]) *
                             ee*self.omega)*(-c1*swdt + c2*cwdt)
                ...
                self.acc[i+1] = -self.omegan2[k] * \
                    self.rd[i+1] - 2.*self.domegan[k]*rv
        ...
        for k in range(0, self.numf):
            ...
            for i in range(0, self.nt[k]):
                ...
                if t <= self.dur:
                    ...
                    rv += -self.domegan[k]*rdf + \
                        self.omegad[k]*ee*(-b1*swdt + b2*cwdt)
