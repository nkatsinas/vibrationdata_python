...
from vibrationdata.vb_utilities import read_two_columns_from_dialog, loglog_plot
from vibrationdata.vb_utilities import spectral_moments_constant_df, interpolate_psd
...
###############################################################################
        ...
        self.hwtext3 = tk.Label(
            top, text='This script calculates the overall level of a PSD.')
        ...
        self.acc_psdutton_read = tk.Button(
            top, text="Read Input File", command=self.read_data)
        self.acc_psdutton_read.config(height=3, width=15)
        self.acc_psdutton_read.grid(
            row=crow, column=0, columnspan=1, padx=0, pady=2, sticky=tk.N)
        ...
        self.tree = Treeview(top, selectmode="extended",
                             columns=("A", "B"), height=6)
        self.tree.grid(row=crow, column=0, columnspan=2,
...
#       Displacement
        ...
        if (self.iunit == 0):
            title_string = 'Displacement PSD   ' + \
                str("%6.3g" % self.drms)+' in RMS Overall '

            ylab = 'Disp (in^2/Hz)'

        else:
            title_string = 'Displacement PSD   ' + \
                str("%6.3g" % self.drms)+' mm RMS Overall '

            ylab = 'Disp (in^2/Hz)'

        fig_num = loglog_plot(self.freq, self.vpsd, xlab,
                              ylab, self.f1, self.f2, title_string, fig_num)


#       Velocity

        if (self.iunit == 0):
            title_string = 'Velocity PSD   ' + \
                str("%6.3g" % self.vrms)+' in/sec RMS Overall '

            ylab = 'Vel ((in/sec)^2/Hz)'

        else:
            title_string = 'Velocity PSD   ' + \
                str("%6.3g" % self.vrms)+' m/sec RMS Overall '
...
#       acceleration
        ...
        title_string = 'Acceleration PSD   ' + \
            str("%6.3g" % self.grms)+' GRMS Overall '

        fig_num = loglog_plot(self.freq, self.acc_psd, xlab,
                              ylab, self.f1, self.f2, title_string, fig_num)
...
###############################################################################
    ...
    def read_data(self):
        ...
        a, b, num = read_two_columns_from_dialog(
